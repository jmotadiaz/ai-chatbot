...
export interface HubAttachmentsPreviewProps {
  files: FilePart[];
  setFiles: React.Dispatch<React.SetStateAction<FilePart[]>>;
  className?: string;
}

export const HubAttachmentsPreview: React.FC<HubAttachmentsPreviewProps> = ({
  files,
  setFiles,
  className,
}) => {
  ...
  return (
    <div
      data-testid="hub-attachments-preview"
    ...
    >
      {files.map((file, idx) => (
        ...
        >
          ...
          {file.loading && (
            <div className="bg-secondary absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 p-1 rounded-full opacity-80 text-xs">
