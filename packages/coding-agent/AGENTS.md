...
- **Entrypoint:** `extensions/superpowers/index.ts` discovers `./skills` via `resources_discover` and appends `USING_SUPERPOWERS_PROMPT` to the system prompt via `pi.on("before_agent_start")` (per-turn, al final). El upstream `context` (user-message prepend) está muerto — ningún adapter consume `transformContext` — así que `before_agent_start` es el reemplazo verificado, ahora en modo append como pediste (antes era prepend, como `context`). Intercambio, no superposición: `src/session-manager.ts` NO añade el bootstrap vía `resourceLoaderOptions.appendSystemPrompt` (solo queda `FILE_REFERENCE_PROMPT`).
