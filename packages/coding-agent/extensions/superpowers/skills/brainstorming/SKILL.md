...
Help turn ideas into fully formed designs and specs through natural collaborative dialogue, using the harness's file browser as the primary review surface.

Start by understanding the current project context, then ask questions one at a time to refine the idea. Once you understand what you're building, write the design to an uncommitted Markdown file for the user to review through the file browser.
...
digraph brainstorming {
    ...
    "Write design as\nuncommitted file" [shape=box];
    "User reviews file\n(via comments)?" [shape=diamond];
    ...
    "Propose 2-3 approaches" -> "Write design as\nuncommitted file";
    "Write design as\nuncommitted file" -> "User reviews file\n(via comments)?";
    "User reviews file\n(via comments)?" -> "Write design as\nuncommitted file" [label="comments"];
    "User reviews file\n(via comments)?" -> "Spec self-review\n(fix inline)" [label="approved"];
    "Spec self-review\n(fix inline)" -> "User approves?";
    "User approves?" -> "Write design as\nuncommitted file" [label="changes requested"];
    "User approves?" -> "Commit spec";
...
> "Spec written (uncommitted) to `<path>`. Review it in the file browser — add comments on specific blocks and send them when ready. I'll commit it once you approve."

Wait for the user's response. If they send comments, address each one by editing the spec and summarise the changes. Repeat until the user approves. Only proceed once the user gives explicit approval.

**Implementation:**

- Once the user approves, commit the spec, then invoke the writing-plans skill to create a detailed implementation plan.
